...
namespace scrprnt
{
    public partial class Form1 : Form
    {
        ...
        /*    private async void button7_Click(object sender, EventArgs e)
           {
               ...
               string filePath = Cred.filePath; // Path to the file you want to upload
        ...
        // Upload without checking whole attachments
        /*  private async void button7_Click(object sender, EventArgs e)
            {
                string jiraBaseUrl = Cred.jiraBaseUrl;
                string issueKey = Cred.issueKey;
              ...
              try
                {
                    ...
                    using (var client = new HttpClient())
                    {
                      ...
                      try
                      {
                          ...
                          {
                              var attachmentFilename = issue.Attachments[0].Filename;
                              Console.WriteLine($"Filename of the first attachment: {attachmentFilename}");

                              //if (jiraClient.IsAttachmentExists(attachmentIds, fileName))
                              if (attachmentFilename == fileName)
                              {
                                  // If the file already exists as an attachment, rename the file and upload it again
                                  string renamedFileName = JiraClient.RenameFile(fileName);
                                  await APIProgram.ConfigureJiraHttpClient(client, jiraBaseUrl, username, password, issueKey);
